import { readLocalStorage, saveLocalStorage, STRAGE_KEY } from "./strage";
...
function init() {
  ...
  memos = readLocalStorage(STRAGE_KEY);
  if (memos.length === 0) {
    //新しいメモを2つ作成する
    memos.push(newMemo());
    memos.push(newMemo());
    //すべてのメモをローカルストレージに保存する
    saveLocalStorage(STRAGE_KEY, memos);
  }
